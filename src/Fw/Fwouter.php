...
class Fwouter
{
	...
	static function init($config = array())
	{
		...
		//$request_url = $request_url . "/" . $default_class;
		...
		do {
			...
			$class_name_with_index = rtrim($class_name, "\\") . "\\Index";
			$debug = $debug . $class_name . ", \n";
			self::db("looking for class: $class_name  (and also: $class_name_with_index)");
			$start--;
		} while (!class_exists($class_name) && !class_exists($class_name_with_index));

		// If the Index Class is found, use that instead
		if (class_exists($class_name_with_index)) {
			$class_name = $class_name_with_index;
		}
		...
		if (!class_exists($class_name)) {
			...
			if (class_exists($base_namespace . $default_class)) {
				...
				$class_obj->match($request_method, $rest_of_url);
				return $class_obj;
			} else {
